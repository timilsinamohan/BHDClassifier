...
def get_baseline():
    ...
    adj_mat_sum = L1["A1"] + L2["A2"] + L3["A3"] + L4["A4"]
    m,n = adj_mat_sum .shape
    X = sparse.lil_matrix((m,n))
    ix = np.nonzero(adj_mat_sum)
    X[ix] = adj_mat_sum[ix] 
    return X
...
def get_rgcn_embeddings():
    ...
    L3 = loadmat('Datasets/LN_2005_2009.mat')
    L4 = loadmat('Datasets/LN_2010_2014.mat')
    ###create the Tensor models###
    m,n = L1["A1"].shape
    L = 4 # number of layers
    ####For layer 0
    X = [sparse.lil_matrix((m,n)) for i in range(L)]
    ix = np.nonzero(L1["A1"])
    source_L1 = [i for i in list(ix[0])]
    target_L1 = [i for i in list(ix[1])]
    rels_L1 = [0]*len(list(ix[0]))
   
    ####For layer 1
    ix = list(np.nonzero(L2["A2"]))
    source_L2 = [i for i in list(ix[0])]
    target_L2 = [i for i in list(ix[1])]
    rels_L2 = [1]*len(list(ix[0]))
    ####For layer 2
    ix = list(np.nonzero(L3["A3"]))
    source_L3 = [i for i in list(ix[0])]
    target_L3 = [i for i in list(ix[1])]
    rels_L3 = [2]*len(list(ix[0]))
    ####For layer 3
    ix = list(np.nonzero(L4["A4"]))
    source_L4 = [i for i in list(ix[0])]
    target_L4 = [i for i in list(ix[1])]
    rels_L4 = [3]*len(list(ix[0]))
    
    source = source_L1 + source_L2 + source_L3 + source_L4
    target = target_L1 + target_L2 + target_L3 + target_L4
    orientation = rels_L1 + rels_L2 + rels_L3 + rels_L4
    ####For layer 3
...
def get_tensor_embeddings():
    ...
    A, R, _, _, _ = rescal_als(X, 10, init='nvecs', conv=1e-3,lambda_A=0.1, lambda_R=0.1)
